...
const FuncTable = () => {
  ...
  const SaveMovie = ()=>{
   let savedMovies = movies.map((value) =>value.id === select?.id ? {...value,url :newImgValue ,name :newInputValue} : value)
                  ...
                  <tr key={index}>
                    ...
                    <td>
                        {select?.id === value.id ?
                         ...
                         filename={file} 
                         onChange={(e)=> setNewImgValue(URL.createObjectURL(e.target.files[0]))}  />
                          :
                      <Img src={value.url} />
                        } 
                    </td>
                    <td>{ select?.id === value.id ?  
                    <Input defaultValue={value.name} 
                     onChange={({target})=> setNewInputValue(target.value) } /> 
                     : value.name}</td>
